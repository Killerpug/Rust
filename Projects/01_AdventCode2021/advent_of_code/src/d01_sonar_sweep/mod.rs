...
pub fn depth_descent_slope_functional2() -> Result<i32, Box<dyn Error + 'static>> {
    ...
    let reader = fs::read_to_string("src/d01_sonar_sweep/input/01_sensor_measurements.txt")?;
    let split = reader.lines().map(|s| s.parse::<i32>().unwrap_or(MIN));
    for line in split {
        if line == MIN {
            //skip this value
            continue;
...
pub fn depth_descent_slope_slide_filter() -> Result<i32, Box<dyn Error + 'static>> {
    ...
    let split = reader.lines().map(|s| s.parse::<i32>().unwrap_or(MIN));
    for line in split {
        result = filter_depth.calculate_sum(line, pos % MAX_MEASUREMENTS);
        pos += 1;
        match result {
            Some(avg) => new_avg = avg,
            None => continue,
        }
        if previous_avg < new_avg {
            // increased depth
            count_increased_depth += 1;
        }
...
impl SlideFilter {
    pub fn new() -> Self {
        ...
        Self {
            slide_window: map,
            measurements_sum: sum,
        }
    }
    /// calculates the sum of 3 last measurements, creating a sliding window.
    pub fn calculate_sum(&mut self, sample: i32, position: u32) -> Option<i32> {
        let old_value = self.slide_window.insert(position, sample);
        let mut sum = 0;

        match self.measurements_sum {
            // filter is working, calculate most recent sum on window(updating only 1 value)
            Some(old_sum) => sum = old_sum - old_value.unwrap() + sample,
            None => {
                if self.slide_window.len() != NUMBER_OF_MEASUREMENTS {
                    // too few samples
                    return None;
                } else {
                    // calculate average for the first time
